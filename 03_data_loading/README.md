...
This module loads source data into the `DATA_LOADING.BRONZE` layer. Bronze data
...
2. Run `data/convert_csv_to_parquet.py` from the `data` directory to generate `product_catalog_source.parquet`. The generated Parquet file is already included when available.
